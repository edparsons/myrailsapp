module Twitter_Text
  module Validation extend self
    ...
    def tweet_length(text, options = {})
      ...
      Twitter_Text::Extractor.extract_urls_with_indices(text) do |url, start_position, end_position|
    ...
    def tweet_invalid?(text)
      ...
      begin
        ...
        return :invalid_characters if Twitter_Text::Regex::INVALID_CHARACTERS.any?{|invalid_char| text.include?(invalid_char) }
    ...
    def valid_username?(username)
      ...
      extracted = Twitter_Text::Extractor.extract_mentioned_screen_names(username)
      # Should extract the username minus the @ sign, hence the [1..-1]
      extracted.size == 1 && extracted.first == username[1..-1]
    end

    VALID_LIST_RE = /\A#{Twitter_Text::Regex[:valid_mention_or_list]}\z/o
    ...
    def valid_hashtag?(hashtag)
      ...
      extracted = Twitter_Text::Extractor.extract_hashtags(hashtag)
    ...
    def valid_url?(url, unicode_domains=true, require_protocol=true)
      ...
      url_parts = url.match(Twitter_Text::Regex[:validate_url_unencoded])
      ...
      return false unless ((!require_protocol ||
                           (valid_match?(scheme, Twitter_Text::Regex[:validate_url_scheme]) && scheme.match(/\Ahttps?\Z/i))) &&
                           valid_match?(path, Twitter_Text::Regex[:validate_url_path]) &&
                           valid_match?(query, Twitter_Text::Regex[:validate_url_query], true) &&
                           valid_match?(fragment, Twitter_Text::Regex[:validate_url_fragment], true))

      return (unicode_domains && valid_match?(authority, Twitter_Text::Regex[:validate_url_unicode_authority])) ||
             (!unicode_domains && valid_match?(authority, Twitter_Text::Regex[:validate_url_authority]))
    end
