...
module Twitter_Text
  ...
  module Extractor extend self
    ...
    def extract_mentions_or_lists_with_indices(text) # :yields: username, list_slug, start, end
      ...
      possible_entries = []
      text.to_s.scan(Twitter_Text::Regex[:valid_mention_or_list]) do |before, at, screen_name, list_slug|
        match_data = $~
        after = $'
        unless after =~ Twitter_Text::Regex[:end_mention_match]
    ...
    def extract_reply_screen_name(text) # :yields: username
      ...
      possible_screen_name = text.match(Twitter_Text::Regex[:valid_reply])
      return unless possible_screen_name.respond_to?(:captures)
      return if $' =~ Twitter_Text::Regex[:end_mention_match]
    ...
    def extract_urls_with_indices(text, options = {:extract_url_without_protocol => true}) # :yields: url, start, end
      ...
      text.to_s.scan(Twitter_Text::Regex[:valid_url]) do |all, before, url, protocol, domain, port, path, query|
        ...
        if !protocol
          next if !options[:extract_url_without_protocol] || before =~ Twitter_Text::Regex[:invalid_url_without_protocol_preceding_chars]
          last_url = nil
          last_url_invalid_match = nil
          domain.scan(Twitter_Text::Regex[:valid_ascii_domain]) do |ascii_domain|
            last_url = {
              :url => ascii_domain,
              :indices => [start_position + $~.char_begin(0),
                           start_position + $~.char_end(0)]
            }
            last_url_invalid_match = ascii_domain =~ Twitter_Text::Regex[:invalid_short_domain]
          ...
          if url =~ Twitter_Text::Regex[:valid_tco_url]
            url = $&
    ...
    def extract_hashtags_with_indices(text, options = {:check_url_overlap => true}) # :yields: hashtag_text, start, end
      ...
      tags = []
      text.scan(Twitter_Text::Regex[:valid_hashtag]) do |before, hash, hash_text|
        match_data = $~
        start_position = match_data.char_begin(2)
        end_position = match_data.char_end(3)
        after = $'
        unless after =~ Twitter_Text::Regex[:end_hashtag_match]
          tags << {
            :hashtag => hash_text,
            :indices => [start_position, end_position]
          }
    ...
    def extract_cashtags_with_indices(text) # :yields: cashtag_text, start, end
      ...
      tags = []
      text.scan(Twitter_Text::Regex[:valid_cashtag]) do |cash_text|
        match_data = $~
